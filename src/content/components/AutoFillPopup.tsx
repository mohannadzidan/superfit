...
export function AutoFillPopup({ focusedField, onClose }: AutoFillPopupProps) {
  const [threadId] = useState(() => `field-filler-${Date.now()}`)
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const suggestFieldValue = tool(async (input) =>{
    ...
    console.log(config)
    providerRegistry.getProvider(config!.providerId)?.configure(config!.providerConfigs[config!.providerId].config)
        ...
        <>
          ...
          {(fieldLabel || hasResume || capturedJob) && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
              ...
              {capturedJob && (
                <ContextBadge
                  label={[capturedJob.jobTitle, capturedJob.companyName].filter(Boolean).join(' @ ')}
                  active={inclJob}
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 6 }}>
            <span style={{ fontSize: 11, color: '#3f3f46' }}>Enter ↵ to fill · Shift+Enter for new line</span>
          </div>
        </>
      )}
    </div>
